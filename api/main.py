...
from src.chatbot_engine import append_forecast_insights_to_response, build_financial_context
...
def compute_forecast(business_id: str, days: int = 7, current_balance: Optional[float] = None) -> Optional[dict]:
    ...
    if "business_id" not in cashflow.columns:
        return None

    business_df = cashflow[cashflow["business_id"].astype(str).str.upper() == biz_id].copy()
    ...
    if "net_cashflow" not in business_df.columns:
        return None

    business_df = business_df.sort_values("date").reset_index(drop=True)
    net_series = business_df["net_cashflow"].astype(float)
    ...
    if len(net_series) > 3:
        std_daily = float(net_series.std())
    else:
        std_daily = abs(base_daily_net) * 0.15
    ...
    elif "cumulative_balance" in business_df.columns:
        starting_balance = float(business_df["cumulative_balance"].iloc[-1])
    else:
        starting_balance = float(business_df["net_cashflow"].cumsum().iloc[-1])
    ...
    daily_forecast = []
    zero_date = None
    last_date = business_df["date"].max() if "date" in business_df.columns else datetime.now()
    ...
    for day_index in range(days):
        ...
        if zero_date is None and balance <= 0:
            zero_date = (last_date + timedelta(days=day_index + 1)).strftime("%Y-%m-%d")
    ...
    min_balance = min(entry["balance"] for entry in daily_forecast)
    final_balance = daily_forecast[-1]["balance"]
    trend = "growing" if base_daily_net > 0 else "declining"

    alert = None
    if min_balance < 0:
        low_day = next(entry["day"] for entry in daily_forecast if entry["balance"] == min_balance)
    ...
    mom_pct = 0.0
    if len(business_df) >= 60:
        last_30_total = business_df.tail(30)["net_cashflow"].sum()
        prev_30_total = business_df.tail(60).head(30)["net_cashflow"].sum()
        if prev_30_total != 0:
            mom_pct = round((last_30_total - prev_30_total) / abs(prev_30_total) * 100, 2)

    summary = f"Projected balance after {days} days: ₹{int(final_balance):,} ({trend})"
    insights = [f"Cashflow change vs prior month: {mom_pct:.1f}%"] if len(business_df) >= 60 else []
...
@app.get("/")
def root():
    return {"service": "FinSense AI", "status": "running", "version": "1.0.0"}
...
def batch_analyze(req: BatchRequest):
    ...
    for transaction in req.transactions:
        try:
            ...
            results.append({"status": "ok", "result": analysis})
        except Exception as exc:
...
def chat_endpoint(req: ChatRequest):
    ...
    system_prompt = "You are FinBot, an expert AI financial assistant for SMBs.\n"
    ...
    try:
        ...
        if financial_summary:
            ...
            category_breakdown = financial_summary.get("category_breakdown", {})
        ...
        if "Here’s what I can infer so far" in context_str:
            reply = (
                ...
                "Insight: "
                + summary.get("onboarding_insight", "Here’s what I can infer so far based on your inputs...")
                + "\n"
...
def get_business_summary(business_id: str):
    ...
    try:
        return generate_business_summary(history, business_id)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
