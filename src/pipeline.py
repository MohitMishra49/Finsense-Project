...
class ModelStore:
    ...
    def load(self, models_dir: str = 'models'):
        ...
        with open(f'{models_dir}/cashflow_stats.json') as f:
            self.cf_stats = json.load(f)
        ...
        arima_path = f'{models_dir}/arima_model.pkl'
...
def _forecast_cashflow(
    ...
    cf_stats = store.cf_stats
    mean_d   = cf_stats.get('mean_daily', 0)
    std_d    = cf_stats.get('std_daily', 1000)

    if store.arima_model and isinstance(store.arima_model, dict):
        # Rolling mean fallback
        history = store.arima_model.get('history', [mean_d]*7)
        weights = np.exp(np.linspace(-1, 0, min(7, len(history))))
        weights /= weights.sum()
        forecast_vals = []
        hist = list(history[-7:])
        for _ in range(days):
            pred = float(np.sum(np.array(hist[-7:]) * weights[-len(hist[-7:]):]))
            forecast_vals.append(pred)
            hist.append(pred)
    elif store.arima_model:
        try:
            forecast_vals = store.arima_model.forecast(steps=days).tolist()
        except Exception:
            forecast_vals = [mean_d] * days
    else:
        # Linear extrapolation with slight noise
        forecast_vals = [
            mean_d + np.random.normal(0, std_d * 0.1)
            for _ in range(days)
        ]
    ...
    daily_forecasts = []
    for i, net in enumerate(forecast_vals):
        running_balance += net
    ...
    if min_balance < 10000:
        low_day = next(d['day'] for d in daily_forecasts
                       if d['balance'] == min_balance)
    ...
    return {
        ...
        'min_balance':    round(float(min_balance), 2),
        'alert':          alert,
        'summary':        (
            f"Projected balance after {days} days: "
            f"₹{daily_forecasts[-1]['balance']:,.0f}"
        ),
    }
...
def analyze_transaction(
    ...
    if current_balance is None:
        current_balance = store.cf_stats.get('last_balance', 50000)

    result['forecast'] = _forecast_cashflow(current_balance, forecast_days)
