...
def forecast_cashflow(
    ...
    if bdf.empty:
        # Derive from transactions if cashflow CSV missing this business
        return _derive_forecast_from_transactions(business_id, current_balance, days)
    ...
    if len(bdf) >= 60:
        ...
        mom_pct  = round((last_30 - prev_30) / abs(prev_30) * 100, 1) if prev_30 != 0 else 0
        mom_str  = f" | Cash flow {abs(mom_pct):.0f}% {'better' if mom_pct > 0 else 'worse'} than prior month"
    else:
        mom_str  = ""
    ...
    return {
        ...
        'alert':            alert,
        'summary':          (
            f"Projected balance after {days} days: ₹{final_balance:,.0f} "
            f"({trend}{mom_str})"
        ),
    }
...
def _derive_forecast_from_transactions(
    ...
    return {
        ...
        'summary':       f"Projected balance after {days} days: ₹{forecasts[-1]['balance']:,.0f} ({trend})",
    }
